...
async def run(*, filters: dict[str, str | int | float], max_pages: int | None) -> None:
...
async def run_presets(*, max_pages: int | None) -> None:
    total_stats = None
    async with ingestion_job() as context:
...
def main() -> None:
    ...
    if args.preset == "all":
        asyncio.run(run_presets(max_pages=args.max_pages))
        return

    filters = _build_filters(args)
    asyncio.run(run(filters=filters, max_pages=args.max_pages))
